#!/usr/bin/env python3
"""
Enhanced Price Collector Service with ET Time Support and Buffering
Features:
- ET (Eastern Time) time support
- In-memory buffer for recent prices
- Improved data cleanup
- Enhanced API with time zone conversion
"""
...
class EnhancedFilePriceStorage:
    ...
    def __init__(self, data_dir: str = 'data', log_dir: str = 'logs', retention_hours: int = 6):
...
class EnhancedPriceCollectorService:
    ...
    def __init__(self, config: Dict):
        ...
        self.storage = EnhancedFilePriceStorage(
            ...
            retention_hours=config.get('data_retention_hours', 6)
        )
    ...
    async def cleanup_task(self):
        ...
        while self.running:
            try:
                ...
                await self.storage.cleanup_old_records(hours=self.config.get('data_retention_hours', 6))
            except Exception as e:
...
class EnhancedPriceAPIServer:
    """Enhanced REST API server with ET time support"""
    ...
    def _setup_routes(self):
        ...
        self.app.router.add_get('/timezones', self.get_time_info)
    ...
    async def get_time_info(self, request):
        """GET /timezones - Get current time in different timezones"""
...
async def main():
    ...
    api_server = EnhancedPriceAPIServer(storage, buffer, config)
