#!/usr/bin/env python3
"""
Test script for the Simple Price Collector
Tests the mock version without web3 dependency
"""
...
import time
import os
from simple_price_service import MockPriceFetcher, FilePriceStorage


async def test_mock_price_fetching():
    """Test fetching mock prices"""
    print("=== Testing Mock Price Fetching ===")
    ...
    fetcher = MockPriceFetcher(config['symbols'])
...
async def main():
    """Run all tests"""
    print("Simple Price Collector Test Suite")
    print("=" * 40)
    print()
    
    try:
        await test_mock_price_fetching()
        await test_file_storage()
        await test_full_cycle()
        
        print("=== Test Summary ===")
        print("✓ All tests completed successfully!")
        print("✓ Mock price fetching works")
        print("✓ File storage system works")
        print("✓ Full fetch & store cycle works")
        print()
        print("The simple price collector is ready to run!")
